...
export default function Village() {
    ...
    queryFn: async ({ pageParam = 1 }) => {
      try {
        const response = await axios.get(
          `${API_CONFIG.BASE_URL}${API_CONFIG.VILLAGES.GET}`,
          {
            params: {
              page: pageParam,
  ...
  // Combine all villages from all pages
  const allVillages = data?.pages.flatMap((page) => page.villages) || [];
          ...
          <>
            ...
            <div ref={loadMoreRef} className="py-4 text-center">
              {isFetchingNextPage ? (
                ...
                <span className="text-sm text-muted-foreground">
                  {allVillages.length}{" "}
                  {`${allVillages.length === 1 ? "village" : "villages"} found`}
                </span>
              )}
            </div>
          </>
