...
export default function Akatsuki() {
    ...
    queryFn: async ({ pageParam = 1 }) => {
      try {
        const response = await axios.get(
          `${API_CONFIG.BASE_URL}${API_CONFIG.AKATSUKI.GET}`,
          {
            params: {
              page: pageParam,
  ...
  // Combine all akatsuki from all pages
  const allAkatsuki = data?.pages.flatMap((page) => page.akatsuki) || [];
  ...
  return (
    <>
      <div className="space-y-10">
        ...
        {!isLoading && !isError && (
          <>
            <CharacterCard data={allAkatsuki} />
